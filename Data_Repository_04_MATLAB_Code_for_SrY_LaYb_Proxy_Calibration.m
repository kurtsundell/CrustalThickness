...
hold on
for t = 1:trials
	[pSrY1(t,:), S1] = polyfit(log(SrYxR(:,t)),SrYyR(:,t),1);
	SrYf1(:,t) = polyval(pSrY1(t,:),log(xax));
	SrYf1R(:,t) = polyval(pSrY1(t,:),log(SrYxR(:,t)));
	...
	Rsquared1(t,1) = 1 - (S1.normr/norm(SrYyR(:,t) - mean(SrYyR(:,t))))^2;
end
pSrY1_med = median(pSrY1);
pSrY1_std = std(pSrY1);
pSrY1_medf1 = polyval(pSrY1_med,log(xax));
...
hold on
for t = 1:trials
	[pLaYb1(t,:), S1] = polyfit(log(LaYbxR(:,t)),LaYbyR(:,t),1);
	LaYbf1(:,t) = polyval(pLaYb1(t,:),log(xax));
	LaYbf1R(:,t) = polyval(pLaYb1(t,:),log(LaYbxR(:,t)));
...
pLaYb1_med = median(pLaYb1);
pLaYb1_std = std(pLaYb1);
pLaYb1_medf1 = polyval(pLaYb1_med,log(xax));
...
hold on
for t = 1:trials
	surffit1 = fit(log([Xr(:,t),Yr(:,t)]),Zr(:,t) ,'poly11');
	p00(t,1) = surffit1.p00;
	p10(t,1) = surffit1.p10;
	p01(t,1) = surffit1.p01;
	SrYLaYbf(:,t) = p00(t,1) + p10(t,1).*log(Xr(:,t)) + p01(t,1).*log(Yr(:,t)); % surfact fit equation z = p00 + p10*x + p01*y
	Resid_SrYLaYbT = Zr(:,t) - SrYLaYbf(:,t);
	if t <= 100 % only plot the first 100 trials or fewer
...
% Turn on for saving figures for AI
%{
pause(15)
...
%% FUNCTION FILES REQUIRED TO RUN THE CODE ABOVE %%
